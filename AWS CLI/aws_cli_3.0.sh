...
# Creación de Gateway de Internet y NAT
# =======================
...
# Creación de Elastic IP para NAT Gateway
...
# Creación de Tabla de Rutas para Subredes Privadas
echo "📍 Creando tabla de rutas para Subredes Privadas..."
RTB_PRIVATE_ID=$(aws ec2 create-route-table --vpc-id $VPC_ID --query 'RouteTable.RouteTableId' --output text)

# Asociar subredes privadas a la nueva tabla de rutas
echo "🔗 Asociando subredes privadas a la tabla de rutas..."
...
# Agregar ruta para acceso a Internet en Subredes Privadas
echo "🚀 Configurando NAT Gateway como salida para subredes privadas..."
aws ec2 create-route --route-table-id $RTB_PRIVATE_ID --destination-cidr-block 0.0.0.0/0 --nat-gateway-id $NAT_GW_ID

# Confirmación
echo "✅ Configuración de NAT Gateway y rutas completada."
